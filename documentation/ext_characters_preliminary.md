...
下列汉字除第 4 组外，*应在 v1.245 版本的「霞鹜文楷」字体下全部显示*。如无特殊标注则按 Unicode 码位排序，**每个字只出现一次。** 原则上只收录简体字，对于下表中含有的繁体字、异体字、日本新字体（日本简化字），后续会进一步整理和筛选，未予收录的字用「〓」表示空位，并在下方按顺序列出未予收录的详细汉字。仅供参考。
...
> ☞ 2.1 字表参考方正近几年发布的简体字体。其中 2.1.1 的汉字见于方正早期的简体字库中，但是是以私用区（PUA）字符的形式存在，近几年发布的字体中已对这些字增加了 Unicode 映射。2.1.2 中

#### 2.1.1 方正早期简体字库中的 PUA 区汉字（按 PUA 区字形排列顺序）

<!--
原始字表：
朮𧮪匽朆𠉂僜𢙐悗挌垽𦭌苳葇蓚**𦶟**<sup>①</sup>𦻘呯𠸁𠾐𤟯獏縴**𩨂**<sup>②</sup>𤆢㭴梹榠𢽾犗𣱝肐痎㾦𤻊䃚眜𥆧𨰾䥺𨱎𨱓皶𦎓䎬聤覅䖬蜰䇲𦨭踄貐𩽼𩾇魌麕𠚤卲㖞㨘璿敠𥄨𨱍䦆
-->
...
〓〓〓〓〓〓〓〓〓佢佷〓偆偍偐偖〓〓傝〓〓〓〓〓兲〓冴劵卍厷〓叡吔吥咁咗咲〓哖啛啲喺嗮嗰嘅嘢〓〓〓嚟〓〓〓囧〓〓〓〓堺〓〓〓〓夋〓奻娸娿媕媺〓〓〓〓寔〓〓〓〓〓〓尛屄屌〓峎崐〓〓〓〓〓〓〓〓〓〓惢〓〓〓〓〓〓〓斲昰〓〓〓〓栢〓〓歘〓〓〓〓〓〓涚淩〓湪〓潶〓〓濛〓炤炰〓〓熯〓牠〓〓珨琍畊〓〓〓睺碁祂禟禩〓籹〓〓〓〓〓〓〓〓羣翋肏〓〓菈菓〓〓薙〓裑〓〓〓〓跩〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓

> 〓 **未予收录的字：** 丄丅丗両亜仏仩伝伱俬偣傚僞僾儍儚児冄収哊嘫嚐嚒囖囘団囨囬圝垨増壊売変夲嬅嬢嬭実寛寳対専尒尓屍崱巻幷広庅庝彮徳応恏慾懐戝抺拝搵斎曽朲朶杺桜楽歩殭毎沢洅涗渋満濆濇瀬炵煖爲犲獣盃県眞秂粧紑紮綉綫緈縄繎莂莣菿蔘衆観訫説赱転郞鈈鈊鉄銆録錵鍒鎚鏂鏄関闇雑靑駄髈髙魜鰂黒
...
## 4. 未被以上分组收录，但作为《新华字典》《现代汉语词典》字头的部分汉字

> ☛ 本组汉字参考「成双酱」整理的常用字表，及《新华字典（第 12 版）》《现代汉语词典（第 7 版）》的字头。
